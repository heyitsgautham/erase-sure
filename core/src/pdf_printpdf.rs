...
impl PdfGenerator {
    ...
    pub fn generate_backup_pdf(
        ...
        current_layer.set_font(&font, 20);
        ...
        self.add_field(&current_layer, &font, "Algorithm", &cert.signature.alg, &mut y_position);
        self.add_field(&current_layer, &font, "Public Key ID", &cert.signature.pubkey_id, &mut y_position);
        let sig_display = self.format_hash(&cert.signature.sig);
